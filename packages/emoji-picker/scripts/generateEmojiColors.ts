...
async function generateEmojiColorMap(emojis: string[]) {
  ...
  for (const emoji of emojis) {
    try {
      ...
      const { dominant } = await sharp(Buffer.from(svg))
        .ensureAlpha()
        .png()
        .stats();
      
      // Convert the dominant color to hex with opacity
      const hexColor = rgbaToHex(dominant.r, dominant.g, dominant.b);
      colorMap[emoji] = hexColor;
      
      console.log(`Processed ${emoji} -> ${hexColor}`);
    } catch (error) {
